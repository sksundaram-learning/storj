...
func (obj *postgresImpl) All_Raw_By_NodeId(ctx context.Context,
	...
	var __embed_stmt = __sqlbundle_Literal("SELECT raws.id, raws.node_id, raws.start_time, raws.end_time, raws.data_total, raws.data_type, raws.created_at, raws.updated_at FROM raws WHERE raws.node_id = ?")
	...
	for __rows.Next() {
		raw := &Raw{}
		err = __rows.Scan(&raw.Id, &raw.NodeId, &raw.StartTime, &raw.EndTime, &raw.DataTotal, &raw.DataType, &raw.CreatedAt, &raw.UpdatedAt)
		if err != nil {
			return nil, obj.makeErr(err)
		}
